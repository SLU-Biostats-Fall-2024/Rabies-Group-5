...
```{r}
rabies1 <- read.csv(here("Data", "Rabies_Laboratory_Submissions.csv"))
```
...
```{r}
# find animal types, when >4 samples, when %pos >50%
rabies %>%
  group_by(Animal.Classification)%>%
    filter (Number.of.Samples >=4 , Positive.Samples/Number.of.Samples > 0.5) 
